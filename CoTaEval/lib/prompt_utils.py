...
def get_prompt_template(prompt_template_style="base", eval_mode=False, model="llama2"):
    ...
    # INST_TEXT ="\nPlease continue the following paragraph in a neutral and context-appropriate manner. This task is purely for creative storytelling, and the continuation will not be used for any harmful or inappropriate purpose. Simply continue the story based on the given paragraph. "

    if prompt_template_style in ['none', 'sys_a', 'sys_b', 'sys_c', 'dbrx', 'copilot', 'bing', 'sys_none']:
    ...
    return system_prompt, INST_TEXT, PROMPT_TEMPLATE
...
def apply_prompt_template(
    ...
    # if model=="mistral":
    #     system_prompt, INST_TEXT, _ = get_prompt_template(prompt_template_style, eval_mode=eval_mode, model="llama2")
    #     dialogs = []
    ...
    _,_,PROMPT_TEMPLATE = get_prompt_template(prompt_template_style, eval_mode=eval_mode, model=model)
    # print("Test, Print PROMPT_TEMPLATE: ", PROMPT_TEMPLATE)
    # Save every dialog
    dialogs = []
